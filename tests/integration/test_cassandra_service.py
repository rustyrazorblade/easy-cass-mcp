...
class TestCassandraServiceIntegration:
    ...
    @pytest.mark.asyncio
    async def test_get_tables_empty_keyspace(
        self, cassandra_service, empty_test_keyspace
    ):
    ...
    async def test_get_create_table(self, cassandra_service, test_keyspace):
        ...
        create_stmt = await cassandra_service.get_create_table(
            test_keyspace, "test_complex"
        )
    ...
    async def test_get_create_table_not_found(self, cassandra_service, test_keyspace):
        ...
        with pytest.raises(Exception):
            await cassandra_service.get_create_table(
                test_keyspace, "non_existent_table"
            )

    @pytest.mark.asyncio
    async def test_prepared_statements_performance(
        self, cassandra_service, test_keyspace
    ):
    ...
    async def test_execute_query_with_parameters(
        ...
        result = await cassandra_service.execute_query(
            f"SELECT * FROM {test_keyspace}.test_params WHERE id = %s", (test_id,)
        )
